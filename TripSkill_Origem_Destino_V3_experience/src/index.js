...
var handlers = {
  ...
  /*  'LaunchRequest': function () {
    ...
    'ServiceIntent': function () {
        ...
        //
        // get parameters into session


        if(this.attributes['state'] != 'service'){
        ...
        if (this.attributes['service'] == 2 || this.attributes['service'] == 3 || this.attributes['service'] == 13){
	        ...
	        if(this.attributes['state'] == 'service'){
	        	this.attributes['state'] = 'serviceSelected'
	        	this.emit(IntentNextAction)
    ...
    NatoIntent : function () {
        ...
        if(this.attributes['state'] != 'origin'){
			this.attributes['NatoA'] = this.event.request.intent.slots.NatoA.value.toString();
	        this.attributes['NatoB'] = this.event.request.intent.slots.NatoB.value.toString();
	        this.attributes['NatoC'] = this.event.request.intent.slots.NatoC.value.toString();
    	}

    	var nato1 = this.attributes['NatoA']
        var nato2 = this.attributes['NatoB']
        var nato3 = this.attributes['NatoC']
    ...
    'IntentNextAction' : function () {
	    ...
	    if (this.event.request.intent.slots.NatoA.value != null && this.event.request.intent.slots.NatoB.value != null && this.event.request.intent.slots.NatoC.value != null) {
	    	this.attributes['NatoA'] =  this.event.request.intent.slots.NatoA.value.toString();
			this.attributes['NatoB'] =  this.event.request.intent.slots.NatoB.value.toString();
			this.attributes['NatoC'] =  this.event.request.intent.slots.NatoC.value.toString();
	    }

	    if (this.event.request.intent.slots.NatoD.value != null && this.event.request.intent.slots.NatoE.value != null && this.event.request.intent.slots.NatoF.value != null) {
	    	this.attributes['NatoD'] =  this.event.request.intent.slots.NatoA.value.toString();
			this.attributes['NatoE'] =  this.event.request.intent.slots.NatoB.value.toString();
			this.attributes['NatoF'] =  this.event.request.intent.slots.NatoC.value.toString();
	    }

		/*
			nato1 =  this.event.request.intent.slots.NatoA.value;
			nato2 =  this.event.request.intent.slots.NatoB.value;
			nato3 =  this.event.request.intent.slots.NatoC.value;
			nato4 =  this.event.request.intent.slots.NatoD.value;
			nato5 =  this.event.request.intent.slots.NatoE.value;
			nato6 =  this.event.request.intent.slots.NatoF.value;
			action1 = this.event.request.intent.slots.ActionA.value;
			action2 = this.event.request.intent.slots.ActionB.value;
		 */  


	  	//
	  	// validate dependency parameters
	    if(this.attributes['state'] != 'service' && this.attributes['state'] != 'serviceSelected' && this.attributes['state'] != 'origin'){
		    if (this.attributes['service'] == null) {

	 			var speechOutput = "Please, first select the service:" + srvlist + "Say service's number"
	        	var repromptSpeech = "Please, first select the service: " + srvlist + "Say service's number"
	        	var header = "Lisbon Trip Skill"
	        	var imageObj = {} 
		    	...
		    	this.attributes['state'] = 'service';
		     	this.emit('ServiceIntent')
				}
			} 


			else if (this.attributes['state'] == 'serviceSelected'){
				if (this.attributes['NatoA'] == null) {

					var speechOutput ="What is your departure station? Say the first three letters using Nato Phonetic Alphabet."
		        	var repromptSpeech = "What is your departure station? Say the first three letters using Nato Phonetic Alphabet."
		        	var header = "Select Departure Station"

		        	this.emit(':askWithCard',  speechOutput, repromptSpeech, header, speechOutput, imageObj);
				}

				else{
					this.attributes['state'] = 'origin';
		     		this.emit('NatoIntent')
				}
			}
   ...
   //handler to return information when the system don t understand
    ...
    /*
    'x' : function () {
    	service = parseInt(this.event.request.intent.slots.Service.value);
    	nato1 =  this.event.request.intent.slots.NatoA.value.toString();
    	nato2 =  this.event.request.intent.slots.NatoB.value.toString();
    	nato3 =  this.event.request.intent.slots.NatoC.value.toString();
    	nato4 =  this.event.request.intent.slots.NatoD.value.toString();
    	nato5 =  this.event.request.intent.slots.NatoE.value.toString();
    	nato6 =  this.event.request.intent.slots.NatoF.value.toString();
    	action1 = this.event.request.intent.slots.ActionA.value;
    	action2 = this.event.request.intent.slots.ActionB.value;
